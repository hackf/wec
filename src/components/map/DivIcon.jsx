...
function DivIconPortal({ children, container, portalId=undefined }) {
  return createPortal(children, container, portalId);
}

export default function DivIcon({ position, color, textColor, number, id }) {
  ...
  return (
    <>
      ...
      <DivIconPortal container={containerRef.current} portalId={`marker-portal-${id}`}>
        <svg
          width="50"
          height="50"
